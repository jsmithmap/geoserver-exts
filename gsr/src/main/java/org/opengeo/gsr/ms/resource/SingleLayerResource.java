...
public class SingleLayerResource extends Resource {
    public static final Variant JSON = new Variant(new MediaType("application/json"));
    private static final Logger LOGGER = org.geotools.util.logging.Logging.getLogger(SingleLayerResource.class);
    public SingleLayerResource(Context context, Request request, Response response, Catalog catalog, String format, String layerId) {
    ...
    private Representation buildJsonRepresentation() {
        ...
        try {
          layerOrTable = LayersAndTables.find(catalog, workspaceName,Integer.valueOf(layerId));
        } catch (Exception e) {
            LOGGER.log(Level.WARNING, "Skipping layer " + 1, e);
        } 
    ...
    private static void encodeLayerOrTable(LayerOrTable layerOrTable, JSONBuilder json) throws IOException {
      if (layerOrTable!=null && layerOrTable.layer.isAdvertised()) {
          LayerInfo layer = layerOrTable.layer;
          json.object();
